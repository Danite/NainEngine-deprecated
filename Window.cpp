...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lparam) {
	...
	if (msg == WM_CREATE) {
		...
		//Retrive the stored " this " pointer
...
bool Window::Initialize() {
	...
	//int iWindowWidth = m_Width + GetSystemMetrics(SM_CXFIXEDFRAME) * 2, iWindowHeight = m_Height + GetSystemMetrics(SM_CYFIXEDFRAME) * 2 + GetSystemMetrics(SM_CYCAPTION);
...
bool Window::ShutDown() {
	...
	//Are we able to relase the DC
	//Set DC to NULL
	if (m_hDC && !ReleaseDC(m_hWindow, m_hDC)) {

		//Logger::Log(_T("Relase DC failed"));
		m_hDC = NULL;
		return false;
	}
